...
### Philosophy
*"Every execution is an opportunity for improvement. Reflect before delivering."*
...
### Philosophy
*"Deterministic tools over probabilistic generation. Always."*
...
async function findMatchingSkill(task: Task): Promise<Skill | null> {
  ...
  return ranked.length > 0 && ranked[0].confidence > 0.7
    ? ranked[0].skill
    : null;
}
```
...
### Philosophy
*"Break down complexity. Plan before acting. Observe before continuing."*
...
async function replan(
  ...
  const adjustedSteps = await adjustSteps(
    originalPlan.steps,
...
**Philosophy**: *"Plan with purpose. Act with precision. Observe with wisdom. Reflect with honesty."*
